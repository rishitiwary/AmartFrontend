...
function Menu(props) {
  ...
  const itemsList = items.map((item, index) => {
    ...
    if (item.SubChildCategories && item.SubChildCategories.length) {
    ...
    if (item.SubChildCategories && item.SubChildCategories.length) {
      submenu = (
        <div className="menu__submenu">
          <Menu items={item.SubChildCategories} />
        </div>
      );
    }
        ...
        {renderLink(
          ...
          <React.Fragment>
            <a
              href={
                item.sub_name
                  ? `shop/catalog/${item.slug}`
                  : `shop/catalog/${item.name}`
              }
            >
              {icon}
              {item.sub_name ? item.sub_name : item.name}
              {arrow}
            </a>
          </React.Fragment>
        )}
