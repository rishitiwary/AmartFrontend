...
function HomePageOne(props) {
  ...
  const PreownedProducts = useProductTabs(
    useMemo(() => [{ id: 1, name: "All", categorySlug: undefined }], []),
    (tab) => shopApi.PreownedProducts({ limit: 8 })
  );
  ...
  console.log("------>", PreownedProducts.data.data);
  return (
    <React.Fragment>
        ...
        {useMemo(
          () => (
            <BlockProductsCarousel
              title="Pre-owned"
              layout="grid-5"
              products={PreownedProducts.data.data}
              loading={PreownedProducts.isLoading}
              groups={PreownedProducts.tabs}
              onGroupClick={PreownedProducts.handleTabChange}
            />
          ),
          [PreownedProducts]
        )}
