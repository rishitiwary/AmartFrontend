...
function WidgetFilters(props) {
    ...
    ({ filter, value }) => {
      ...
      if (handler) {
        dispatch({
          ...
          filter: filter.slug,
          value: handler.isDefaultValue(filter, value)
            ? undefined
  ...
  if (filters && filters.length) {
    filtersList = filters.map((filter) => {
      ...
      if (handler && filter.slug in values) {
        value =
          handler.deserialize(values[filter.slug]) ||
          handler.getDefaultValue(filter);
      }
      ...
      return (
        <div
          key={filter.slug}
          className={filter.checkVal ? "widget-filters__item" : "d-none"}
        >
          <Collapse
  ...
  return (
    <div className={classes}>
      <h3 className="widget-filters__title widget__title">{title}</h3>
